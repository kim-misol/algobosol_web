...
for r in rows:
    ...
    data.append({
        ...
        "현재가": cur,
        "[현재가] [매수가]": f"{cur} / {r.buy_price}" if (cur is not None and r.buy_price) else "",
...
display_cols = [
    "날짜","종목","매수 의견 (익절 +30%)","투자 포인트","현재가",
    "[현재가] [매수가]","[현재가 - 매수가]","수익률","수량","매입비중","매도완료","상세보기"
]
...
edited = st.data_editor(
    ...
    column_config={
        ...
        "매수 의견 (익절 +30%)": st.column_config.NumberColumn("매수 의견 (익절 +30%)", step=10.0),
        "투자 포인트": st.column_config.TextColumn("투자 포인트"),
        "현재가": st.column_config.NumberColumn("현재가", disabled=True),
        "[현재가] [매수가]": st.column_config.TextColumn("[현재가] [매수가]", disabled=True),
        "[현재가 - 매수가]": st.column_config.NumberColumn("[현재가 - 매수가]", disabled=True),
...
if st.button("💾 저장", type="primary"):
    ...
    for _, r in edited.iterrows():
        pts = [x.strip() for x in str(r["투자 포인트"] or "").replace("·", "\n").replace(",", "\n").split("\n") if x.strip()]
        payload.append(AnalysisUpdate(
            id=int(r["id"]),
            date=str(r["날짜"]).strip(),
            name=str(r["종목"]).strip(),
            buy_price=(float(r["매수 의견 (익절 +30%)"]) if r["매수 의견 (익절 +30%)"] not in (None, "") else None),
            invest_points=pts,
